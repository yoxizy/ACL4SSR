...
function submit_phone_order() {
  console.log("提交订单中...");
  var submitBtnContainer = $("div[view-name='BottomSubmitBtn']");
  if (submitBtnContainer.length === 0) {
    setTimeout(submit_phone_order, 200);
    return;
  }
  var submitBtn = submitBtnContainer.find("div[style*='background-color']");
  if(submitBtn.length > 0) {
      var myEvent = new Event("dx_tap");
      submitBtn[0].dispatchEvent(myEvent);
  }
...
function check_phone_alert() {
  ...
  if (mian.length > 0 && mian.html().indexOf("系统繁忙") != -1) {
      window.location.reload();
      return;
  }
 
  // If no alert and not busy, retry submission logic
  setTimeout(submit_phone_order, 300);
 
  // Stop refreshing after 30 seconds to prevent infinite loops
  setTimeout(() => {
    clearTimeout(window.timer);
    sessionStorage.clear();
  }, 30000);
